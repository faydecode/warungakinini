...
class C_admin_jabatan extends CI_Controller {
	...
	public function simpan()
	{
		if (!empty($_POST['stat_edit']))
		{
			...
			header('Location: '.base_url().'admin-jabatan?'.http_build_query($_GET));
		}
		else
		{
	...
	public function list_hak_akses()
	{
		...
		if((!empty($_GET['cari'])) && ($_GET['cari']!= "")  )
		{
			$cari = "WHERE A.kode_kantor = '".$this->session->userdata('ses_kode_kantor')."' AND  A.nama_fasilitas LIKE '%".str_replace("'","",$_GET['cari'])."%'";
		}
		else
		{
