...
class C_admin_kat_produk extends CI_Controller {
	...
	public function index()
	{
		...
		else
		{
			...
			if(!empty($cek_ses_login))
			{
				...
				$config['first_url'] = site_url('admin-kat-produk?'.http_build_query($_GET));
				$config['base_url'] = site_url('admin-kat-produk/');
	...
	public function simpan()
	{
		if (!empty($_POST['stat_edit']))
		{
			...
			header('Location: '.base_url().'admin-kat-produk?'.http_build_query($_GET));
		}
		else
		{
